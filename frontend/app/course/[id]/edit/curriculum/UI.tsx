// 코드의 길이가 긴 관계로 서버처리 로직과 렌더링 로직을 분리함.
"use client";
...
export default function EditCurriculumUI({
  initialCourse,
}: {
  ...
  const [editLecture, setEditLecture] = useState<LectureDto | null>(null);
  const [isEditLectureDialogOpen, setIsEditLectureDialogOpen] = useState(false);

  // 코스 데이터 조회
  const { data: course } = useQuery<CourseResponseDto>({
  ...
  const toggleLecturePreviewMutation = useMutation({
    mutationFn: async (lecture: LectureDto) => {
  ...
  const handleToggleLecturePreview = (lecture: LectureDto) => {
  ...
  return (
    <div className="space-y-8 flex flex-col items-center">
      ...
      {course.sections?.map((section: SectionDto, sectionIdx: number) => (
        <div key={section.id} className="border rounded-lg p-4 bg-white w-full">
          ...
          <div className="space-y-2 mt-4">
            {section.lectures?.map(
              (lecture: LectureDto, lectureIdx: number) => (
                <div
                  key={lecture.id}
